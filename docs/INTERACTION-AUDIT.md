...
| `alert()` success/info: playlist load, settings saves, template resets, streamer-file checks | A | Migrate harmless transient results individually. Playlist-load success is the 11a proof. | 11b |
...
Audit baseline found 31 `alert()` call sites and 4 `confirm()` call sites. After the single Slice 11a playlist-success migration, 30 `alert()` call sites and all 4 `confirm()` call sites remain. They intentionally are not bulk-migrated in this slice.
