...
| Repeated pure Twitch live-status failures | D | Slice 12a groups only error statuses without a recording lifecycle into a compact disclosure; recording failures remain prominent and offline stays separate. Polling and backend behavior are unchanged. | 12a |

Final UI/UX redesign milestone (Slice 12b): reduced redundant idle/zero-state messaging across Dashboard, Queue, Find VODs, and Local VODs, and made exceptional streamer validation states more prominent than ordinary valid rows. Workflow semantics remain unchanged.
