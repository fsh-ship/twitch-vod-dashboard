...
Audit baseline found 31 `alert()` call sites and 4 `confirm()` call sites. Slice 11a migrated playlist-load success; Slice 11b1 migrated the queue-start acknowledgement and two harmless template-reset acknowledgements; Slice 11b2 migrated playlist-refresh and read-only streamer-file-check failures. The current baseline is 25 `alert()` call sites and 4 `confirm()` call sites. Remaining alerts are intentionally retained where they carry validation, diagnostics, persistent form state, operational failure, OAuth/security detail, or destructive-flow context.
